...
export async function POST(request: NextRequest, { params }: { params: { journeyId: string; lessonId: string } }) {
  try {
    ...
    // Obter a posição do vídeo do corpo da requisição
    ...
    if (progressResult.rows.length === 0) {
      ...
      await query("INSERT INTO progress (user_id, lesson_id, last_watched_position) VALUES ($1, $2, $3)", [
        user.id,
        lessonId,
        position,
      ])
      ...
      await query(
        "UPDATE progress SET last_watched_position = $1, updated_at = CURRENT_TIMESTAMP WHERE user_id = $2 AND lesson_id = $3",
        [position, user.id, lessonId],
      )
    }
